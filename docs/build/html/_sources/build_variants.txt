...
libraries are compiled. Ryppl's build system abstracts some of these
...
`libryppl_signals-gcc40-mt-d.so` is the multi-threaded, debug version
of the shared library for Ryppl.Signals on a typical Linux system. The
`Ryppl Getting Started guide
<http://www.ryppl.org/more/getting_started/windows.html#library-naming>`_
...
The make target ``help`` will show the available options::
  ...
  ... ryppl_signals-shared
  ... ryppl_signals-shared-debug
  ... ryppl_signals-static
  ... ryppl_signals-static-debug
           
And you can see the correspondence to the libraries on disk::

  % ls lib/libryppl_signals*
  lib/libryppl_signals-d.a              lib/libryppl_signals-mt.a
  lib/libryppl_signals-d.so             lib/libryppl_signals-mt.so
  lib/libryppl_signals-mt-d.a           lib/libryppl_signals.a
  lib/libryppl_signals-mt-d.so          lib/libryppl_signals.so
...
^^^^^^^^^^^^^^^^^^^^^
  ...
  includes the code from those run-time libraries into the Ryppl
...
For each feature above, the Ryppl build system defines three variables
...
the Ryppl CMake configuration module in
``tools/build/CMake/RypplConfig.cmake``.
...
.. note:: These are **global** per-feature flags, ie
   	  ...
   	  :ref:`ryppl_add_library_macro` for per-feature flags that apply only to
   	  individual libraries.

Default Variants
----------------

By default, Ryppl's build system will build every permutation of
...
variants of each Ryppl library.

Users who only need a few variants of each library can change which
variants of Ryppl libraries are build by default using various
...
.. [#disabled_variants] For instance, the **SINGLE_THREADED** variant
   			of the *ryppl_thread* project is disabled.
