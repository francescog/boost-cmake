...
.. index:: ryppl_add_library 
.. _ryppl_add_library_macro:

ryppl_add_library
-----------------

This macro creates a new Ryppl library target that generates a compiled library
...
.. cmake:: ryppl_add_library(libname source1 source2 ...)
...
^^^^^^^
    ...
    States that this Ryppl libraries depends on and links
...
The Ryppl.Thread library binary is built using the following
invocation of the `ryppl_add_library` macro. The options passed to the
macro indicate that CMake should define `RYPPL_THREAD_BUILD_DLL` to 1
when building shared libraries and `RYPPL_THREAD_BUILD_LIB` to 1 when
