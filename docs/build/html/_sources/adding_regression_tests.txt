...
This page describes how to add regression tests for a Ryppl library in
the CMake-based build system. Before adding regression tests, make
sure you have already followed the directions for
:ref:`ryppl_library_project_macro` , so that the CMake system
recognizes your Ryppl library project, and (if necessary)
...
:ref:`ryppl_library_project_macro`. Follow these steps to add this new
library into Ryppl's build system. If your library has multiple
...
#.  If your regression test depends on any other part of ryppl then
    you will need to inform the build system of such with the
    following line::

      ryppl_additional_test_dependencies(libname RYPPL_DEPENDS test fusion)
...
#.  For each test that only needs to be compiled (but not executed),
    ...
    :ref:`ryppl_test_compile` or :ref:`ryppl_test_compile_fail`
    macros, respectively. The most basic usage of these macros
    provides only the test name, e.g., ::

      ryppl_test_compile(compile_test)
      ryppl_test_compile_fail(compile_fail_test)
...
3.  For any tests that need to be built and executed, use the
    :ref:`ryppl_test_run` or :ref:`ryppl_test_run_fail` macros. Both
    tests will build, link and execute a regression test. The
    :ref:`ryppl_test_run` macro expects that executable to return an
    exit code of zero, while the :ref:`ryppl_test_run_fail` macro
    ...
    source file ``simple_test.cpp``::
      ...
      ryppl_test_run(simple_test)

    Often, we'll want to link against our own Ryppl library, which we
    do using the ``DEPENDS`` argument to ``ryppl_test_run``::

       ryppl_test_run(big_test big_test1.cpp big_test2.cpp
         DEPENDS ryppl_libname-static
         )
     
    Here, we have created a test ``big_test``, built from the source
    files ``big_test1.cpp`` and ``big_test2.cpp``, which will link
    against the static library for ``ryppl_libname``. We could create
    a similar test that links against the shared library for
    ``ryppl_libname``, passing along compilation flags specific to the
    shared library::

      ryppl_test_run(big_test_dll big_test1.cpp big_test2.cpp
        DEPENDS ryppl_libname-shared
        COMPILE_FLAGS "-DRYPPL_LIBNAME_DYN_LINK=1"
        )

    Some tests require command-line arguments. For example, say we
    want to pass ``-loop 1000`` to a randomized test. We can do so
    using the ``ARGS`` argument to ``ryppl_test_run`` (or
    ``ryppl_test_run_fail``)::

        ryppl_test_run(random_test ARGS "-loop" "1000" DEPENDS ryppl_libname-static)
