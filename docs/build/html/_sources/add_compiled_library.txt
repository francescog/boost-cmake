...
1. Create a new file ``libs/libname/src/CMakeLists.txt`` with your
   ...
   invocations of the :ref:`ryppl_add_library_macro` macro, which adds a
   compiled Ryppl library to the CMake system. This macro provides the
   ...
   ``ryppl_libname`` library from the source files ``mysrc1.cpp`` and
...
2. For simple libraries, that's it! Rebuilding via CMake (e.g.,
   running ``make`` or reloading and rebuilding the Ryppl project in
   your IDE) will build the new library, including several different
   variants for different compilation options. Your Ryppl library will
   also be included when the user installs Ryppl or builds a binary
   package of Ryppl. Within the CMake configuration, you will also see
   an option ``BUILD_LIBNAME``, which allows the user to decide
   whether or not to build this Ryppl library.

3. Many libraries will need specific compilation options when
   building, need to link against other libraries (Ryppl or
...
``BUILDING_RYPPL_LIBNAME`` when building the library. We can do so by
passing the ``COMPILE_FLAGS`` option to ``ryppl_add_library``::

  ryppl_add_library(libname
      mysrc1.cpp mysrc2.cpp
      COMPILE_FLAGS "-DBUILDING_RYPPL_LIBNAME=1"
      )

Now when CMake builds the library, it will pass the flag
``-DBUILDING_RYPPL_LIBNAME=1`` to the compiler.
...
define a macro (say, ``RYPPL_LIBNAME_DYN_LINK``) when building the
...
variant-specific compile flags, e.g., ::
  ...
  ryppl_add_library(libname
      mysrc1.cpp mysrc2.cpp
      COMPILE_FLAGS "-DBUILDING_RYPPL_LIBNAME=1"
      SHARED_COMPILE_FLAGS "-DRYPPL_LIBNAME_DYN_LINK=1"
      )
...
:ref:`ryppl_add_library_macro` reference for more
information.

Linking to Other Ryppl Libraries
--------------------------------

Some Ryppl libraries depends on other Ryppl libraries. For example,
perhaps our library uses the Ryppl.Filesystem library under the
hood. We can use the ``DEPENDS`` feature of the
:ref:`ryppl_add_library_macro` to state which libraries our library
depends on. In this example, we'll link against ``ryppl_filesystem``::
  ...
  ryppl_add_library(libname
      ...
      COMPILE_FLAGS "-DBUILDING_RYPPL_LIBNAME=1"
      SHARED_COMPILE_FLAGS "-DRYPPL_LIBNAME_DYN_LINK=1"
      DEPENDS ryppl_filesystem
      )

Now, each variant of the ``ryppl_libname`` library will link against
the appropriate ``ryppl_filesystem`` library variant. Whenever
``ryppl_filesystem`` changes, our library will be relinked
appropriately.

Linking External Libraries/Optional Sources
-------------------------------------------

Sometimes, Ryppl libraries need to link against other libraries
...
:ref:`ryppl_add_library_macro`. Using these features together, we can
...
PNG support into our library::
  ...
  ryppl_add_library(libname
    mysrc1.cpp mysrc2.cpp
    ${EXTRA_SOURCES}
    COMPILE_FLAGS "-DBUILDING_RYPPL_LIBNAME=1 ${PNG_DEFINITIONS}"
...
the Ryppl-specific examples, such as
http://svn.ryppl.org/svn/ryppl/branches/release/tools/build/CMake/FindICU.cmake
For a real-life example of finding system libraries and using that
information to add optional, extra capabilities to a Ryppl library,
check out the build rules for the Ryppl.IOStreams library at
http://svn.ryppl.org/svn/ryppl/branches/release/libs/iostreams/src/CMakeLists.txt
...
The Ryppl build system defines many different :ref:`VARIANTS`, which
...
different variants of each Ryppl library.
...
to :ref:`ryppl_add_library_macro` stating which features should the library
