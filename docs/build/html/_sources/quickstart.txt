...
This page describes how to configure and build Ryppl with CMake. By
following these instructions, you should be able to get CMake,
configure a Ryppl build tree to your liking with CMake, and then
build, install, and package Ryppl libraries.
...
.. note::
  ...
  In these instructions, we will do things such that the Ryppl source
  tree (with CMake build files) is available in the directory
  ``$RYPPL/src`` and that the build will happen in ``$RYPPL/build``::
  ...
  variable ``RYPPL``, this is a convention used in this document.
...
Ryppl.CMake is distributed *separately* from upstream ryppl.  Code is
...
the tag::
  ...
  git clone git://gitorious.org/ryppl/cmake.git src
  cd src
  git checkout <TAG>
...
Run the CMake configuration program, providing it with the Ryppl
source directory::

  cmake -DCMAKE_INSTALL_PREFIX=/somewhere $RYPPL/src 

(:ref:`CMAKE_INSTALL_PREFIX` defaults to ``/usr/local`` on unix and
``C:\\Program Files\Ryppl`` on windows).  Replace ``/somewhere`` above
...
from ``cmake``.  It looks somewhat like this::
  ...
  -- Configuring done
  -- Generating done
  -- Build files have been written to: $RYPPL/build

The directory ``$RYPPL/build`` should now contain a bunch of generated
...
Now build and install ryppl::

  make install

You'll see::

  Scanning dependencies of target ryppl_date_time-mt-shared
  [  0%] Building CXX object libs/date_time/src/CMakeFiles/ryppl_date_time-mt-shared.dir/gregorian/greg_month.cpp.o
  [  0%] Building CXX object libs/date_time/src/CMakeFiles/ryppl_date_time-mt-shared.dir/gregorian/greg_weekday.cpp.o
  [  1%] Building CXX object libs/date_time/src/CMakeFiles/ryppl_date_time-mt-shared.dir/gregorian/date_generators.cpp.o
  Linking CXX shared library ../../../lib/libryppl_date_time-mt.so
  [  1%] Built target ryppl_date_time-mt-shared
  ...
  -- Installing: /tmp/flanryppl/lib/libryppl_wave-mt-d.a
  -- Installing: /tmp/flanryppl/lib/libryppl_wave-mt-d.so
  -- Removed runtime path from "/tmp/flanryppl/lib/libryppl_wave-mt-d.so"
  -- Installing: /tmp/flanryppl/bin/bcp
  -- Installing: /tmp/flanryppl/bin/inspect

And you're done. Once the build completes (which make take a while, if
you are building all of the Ryppl libraries), the Ryppl libraries will
...
* Use the *Browse...* button next to *Where is the source code* to
  point CMake at the Ryppl source code in ``$RYPPL\src``.
* Use the second *Browse...* button to next to *Where to build the
  binaries* to select the directory where Ryppl will build binaries,
  ``$RYPPL\build``.
* Click *Configure* a first time to configure Ryppl, which will search
...
* Finally, click *Generate* to generate project files.  ``Ryppl.sln``,
...
^^^^^^^^^^^^^^^^^^^^^^
  ...
  Start up Visual Studio, load the solution or project ``Ryppl`` from
  the Ryppl build directory you set in the CMake configuration
  earlier. Then, just click *Build* to build all of Ryppl.
...
The installation of Ryppl's headers and compiled libraries uses the
same tools as building the library. With Microsoft Visual Studio, just
load the Ryppl solution or project and build the 'INSTALL' target to
