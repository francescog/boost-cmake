...
like this::
   ...
   RYPPL_PLATFORM                   linux 
   BUILD_BCP                        ON 
   BUILD_RYPPL_WSERIALIZATION       ON 
   BUILD_EXAMPLES                   NONE 
   BUILD_INSPECT                    ON 
   BUILD_TESTS                      NONE 
...
above.  An excerpt of this file::
  ...
  //
  // Mangle lib names for windows, e.g., ryppl_filesystem-gcc41-1_34
  //
...
compiler name/version and ryppl version.  For example, with
``BUILD_VERSIONED`` set to ``OFF``, the signals library looks like
this::

  % ls lib/*signals*
  lib/libryppl_signals-mt-d.a    lib/libryppl_signals-mt.a
  lib/libryppl_signals-mt-d.so*  lib/libryppl_signals-mt.so*
  
But with it on, (on a gcc 4.3 linux box)::

  % ls lib/*signal*
  lib/libryppl_signals-gcc43-mt-1_40.a    
  lib/libryppl_signals-gcc43-mt-d-1_40.a
  lib/libryppl_signals-gcc43-mt-1_40.so*  
  lib/libryppl_signals-gcc43-mt-d-1_40.so*
...
.. note:: The on-disk names of library :ref:`variants <variants>` are
   	  ...
   	  ``WINMANGLED_LIBNAMES`` affects mangling of compiler and ryppl
   	  version only.
...
See also the :ref:`ryppl_library_project_macro` macro.  
...
.. note::
   ...
   like this::
      ...
      CMake Error at tools/build/CMake/RypplCore.cmake:736 (get_property):
        get_property could not find TARGET ryppl_system-mt-shared.  Perhaps it has
        not yet been created.
      Call Stack (most recent call first):
        tools/build/CMake/RypplCore.cmake:1170 (ryppl_library_variant)
        libs/filesystem/src/CMakeLists.txt:7 (ryppl_add_library)
...
argument to the :ref:`ryppl_library_project_macro` macro.
...
``$RYPPL_ROOT/tools/``) that should be built, e.g.::
...
this is on::
  ...
  % ls -l libryppl_thread*.so*
  lrwxrwxrwx 1 troy troy     30 Oct 29 18:37 libryppl_thread-mt-d.so -> libryppl_thread-mt-d.so.1.41.0*
  -rwxr-xr-x 1 troy troy 571361 Oct 29 18:37 libryppl_thread-mt-d.so.1.41.0*
  lrwxrwxrwx 1 troy troy     28 Oct 29 18:37 libryppl_thread-mt.so -> libryppl_thread-mt.so.1.41.0*
  -rwxr-xr-x 1 troy troy 114963 Oct 29 18:37 libryppl_thread-mt.so.1.41.0*
  
  % readelf -a libryppl_thread-mt.so | grep SONAME
   0x000000000000000e (SONAME)             Library soname: [libryppl_thread-mt.so.1.41.0]
      
and if off::

  % ls -l lib/*signals*
  -rwxr-xr-x 1 troy troy  835522 Oct 29 15:10 lib/libryppl_signals-mt-d.so*
  -rwxr-xr-x 1 troy troy  121886 Oct 29 15:10 lib/libryppl_signals-mt.so*
