...
This is the first cmake beta based on upstream ``Ryppl_1_41_0_beta1``.
...
* Special targets for the ryppl-cmake maintainer
...
This release (as released by upstream Ryppl) does **not** contain
...
* Docs tagged with specific ryppl-cmake release version.

1.40.0.cmake1
-------------

From the ryppl-cmake list::

  > As of now, your Ryppl 1.40.0 branch builds and installs without error 
...
.. warning:: -DCMAKE_IS_EXPERIMENTAL=ORLY_YARLY
   ...
   This guard variable is included in releases of Ryppl.CMake through
...
Ryppl.CMake was included as an experimental system for the first time.
It is perfectly capable of doing the basic build and install of ryppl.
You *must* pass the argument ::

  -DCMAKE_IS_EXPERIMENTAL=ORLY

to the initial run of cmake, or you will see an intimidating message
explaining that Ryppl.CMake != Ryppl.Build.  It looks like this::

  -- ##########################################################################
  -- 
  --               Only Ryppl.Build is officially supported.
  -- 
  --                       This is not Ryppl.Build.
  -- 
  ...
  --  
  --  For more information on ryppl-cmake see the wiki:
  --      https://svn.ryppl.org/trac/ryppl/wiki/CMake
  -- 
  --  Subscribe to the mailing list:
  --      http://lists.ryppl.org/mailman/listinfo.cgi/ryppl-cmake
  -- 
  --  NOTE:  Please ask questions about this build system on the ryppl-cmake list,
  --         not on other ryppl lists.
  -- 
  --  And/or check the archives:
  --      http://news.gmane.org/gmane.comp.lib.ryppl.cmake
  -- 
  -- ##########################################################################
...
::
  ...
  % mkdir /tmp/ryppl
  % cd /tmp/ryppl
  % svn co https://svn.ryppl.org/svn/ryppl/tags/release/Ryppl_1_38_0 src
  % mkdir build
  % cd build
  % cmake -DCMAKE_IS_EXPERIMENTAL=ORLY -DCMAKE_INSTALL_PREFIX=/path/to/installdir ../src

At this point, you have two options: you either want to leave ryppl in
...
**In-place**
  ...
  and your libraries will be in /tmp/ryppl/build/lib, and the headers in
  /tmp/ryppl/src, (where you'd expect them to be).
  
**Installed to some location**

  This will install ryppl to ``lib/`` and ``include/`` under the
...
* There isn't much support for building/running tests within ryppl in
  these releases.
* In version 1.39, the ``RYPPL_VERSION_MINOR`` is wrong: it is set to
  1.38.  You can set this manually by looking for
  ``RYPPL_VERSION_MINOR`` in the toplevel ``CMakeLists.txt``
* The ryppl build names the ``ryppl_prg_exec_monitor`` and
  ``ryppl_unit_test_framework`` libraries with an additional trailing
...
There was a CMake branch that built these releases, but Ryppl.CMake
