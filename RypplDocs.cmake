##########################################################################
# Ryppl Documentation Generation                                         #
##########################################################################
...
# Use Doxygen to parse header files and produce RypplBook output.
#
#   doxygen_to_rypplbook(output header1 header2 ...
#     [PARAMETERS param1=value1 param2=value2 ... ])
#
# This macro sets up rules to transform a set of C/C++ header files
# into RypplBook reference documentation. The resulting RypplBook XML
...
# ryppl_add_documentation.
macro(doxygen_to_rypplbook OUTPUT)
  ...
  set(THIS_DOXY_HEADER_PATH ${CMAKE_SOURCE_DIR}/src/${libname}/include)
  ...
  add_custom_command(OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/xml/index.xml
    ...
    COMMENT "Generating Doxygen XML output for Ryppl.${RYPPL_PROJECT_NAME}..."
  ...
  xsl_transform(
    ...
    COMMENT "Collecting Doxygen XML output for Ryppl.${RYPPL_PROJECT_NAME}...")

  # Transform single Doxygen XML file into RypplBook XML
  xsl_transform(${OUTPUT}
    ${CMAKE_CURRENT_BINARY_DIR}/xml/all.xml
    STYLESHEET ${RYPPLBOOK_XSL_DIR}/doxygen/doxygen2rypplbook.xsl
    COMMENT "Transforming Doxygen XML into RypplBook XML for Ryppl.${RYPPL_PROJECT_NAME}...")
endmacro(doxygen_to_rypplbook)

# Adds documentation for the current library or tool project
#
#   ryppl_add_documentation(source1 source2 source3 ...
...
# 
#   QuickBook
#   RypplBook (.XML extension):
macro(ryppl_add_documentation SOURCE)
  ...
  # documentation, do so now and produce the requested RypplBook XML
  # file.
  if (THIS_DOC_HEADERS)
    set(DOC_HEADER_FILES)
    set(DOC_RYPPLBOOK_FILE)
    foreach(HEADER ${THIS_DOC_HEADERS})
      ...
      if (HEADER_EXT STREQUAL ".XML")
        if (DOC_RYPPLBOOK_FILE)
          # Generate this RypplBook file from the headers
          doxygen_to_rypplbook(
            ${CMAKE_CURRENT_BINARY_DIR}/${DOC_RYPPLBOOK_FILE}
            ${DOC_HEADER_FILES}
            PARAMETERS ${THIS_DOC_DOXYGEN_PARAMETERS})
          list(APPEND THIS_DOC_DEFAULT_ARGS 
            ${CMAKE_CURRENT_BINARY_DIR}/${DOC_RYPPLBOOK_FILE})
        endif()
        set(DOC_RYPPLBOOK_FILE ${HEADER})
        set(DOC_HEADER_FILES)
      else()
        if (NOT DOC_RYPPLBOOK_FILE)
          message(SEND_ERROR 
            "HEADERS argument to ryppl_add_documentation must start with a RypplBook XML file name for output")
        endif()
  ...
  if (THIS_DOC_EXT STREQUAL ".QBK")
    if (BUILD_QUICKBOOK)
      # Transform Quickbook into RypplBook XML
      get_filename_component(SOURCE_FILENAME ${SOURCE} NAME_WE)
      set(RYPPLBOOK_FILE ${SOURCE_FILENAME}.xml)
      add_custom_command(OUTPUT ${RYPPLBOOK_FILE}
        COMMAND quickbook "--output-file=${RYPPLBOOK_FILE}"
        ${THIS_DOC_SOURCE_PATH} 
        DEPENDS ${THIS_DOC_SOURCE_PATH} ${THIS_DOC_DEFAULT_ARGS}
        COMMENT "Generating RypplBook documentation for Ryppl.${RYPPL_PROJECT_NAME}...")

      # Transform RypplBook into other formats
      ryppl_add_documentation(${CMAKE_CURRENT_BINARY_DIR}/${RYPPLBOOK_FILE})
    else()
      message(SEND_ERROR 
        "Quickbook is required to build Ryppl documentation.\nQuickbook can be built by enabling the BUILD_QUICKBOOK.")
    endif()
  elseif (THIS_DOC_EXT STREQUAL ".XML")
    # Transform RypplBook XML into DocBook XML
    get_filename_component(SOURCE_FILENAME ${SOURCE} NAME_WE)
    set(DOCBOOK_FILE ${SOURCE_FILENAME}.docbook)
    xsl_transform(${DOCBOOK_FILE} ${THIS_DOC_SOURCE_PATH} 
      ${THIS_DOC_DEFAULT_ARGS}
      STYLESHEET ${RYPPLBOOK_XSL_DIR}/docbook.xsl
      CATALOG ${CMAKE_BINARY_DIR}/catalog.xml
      COMMENT "Generating DocBook documentation for Ryppl.${RYPPL_PROJECT_NAME}..."
      MAKE_TARGET ${RYPPL_PROJECT_NAME}-docbook)

    # Transform DocBook into other formats
    ryppl_add_documentation(${CMAKE_CURRENT_BINARY_DIR}/${DOCBOOK_FILE})
  elseif(THIS_DOC_EXT STREQUAL ".DOCBOOK")
    ...
    if (BUILD_DOCUMENTATION_HTML)
      xsl_transform(
        ...
        ${THIS_DOC_SOURCE_PATH} 
        STYLESHEET ${RYPPLBOOK_XSL_DIR}/html.xsl
        CATALOG ${CMAKE_BINARY_DIR}/catalog.xml
        DIRECTORY HTML.manifest
        PARAMETERS admon.graphics.path=images
                   navig.graphics.path=images
                   ryppl.image.src=ryppl.png
        COMMENT "Generating HTML documentaiton for Ryppl.${RYPPL_PROJECT_NAME}..."
        MAKE_TARGET ${RYPPL_PROJECT_NAME}-html)

      add_custom_command(TARGET ${RYPPL_PROJECT_NAME}-html
	POST_BUILD
	COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_SOURCE_DIR}/doc/src/rypplbook.css ${CMAKE_CURRENT_BINARY_DIR}/html
	COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_SOURCE_DIR}/ryppl.png ${CMAKE_CURRENT_BINARY_DIR}/html
	)
      # Install generated documentation
      install(DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}/html 
        DESTINATION share/ryppl-${RYPPL_VERSION}
    ...
    if (BUILD_DOCUMENTATION_MAN_PAGES)
      xsl_transform(
        ...
        ${THIS_DOC_SOURCE_PATH} 
        STYLESHEET ${RYPPLBOOK_XSL_DIR}/manpages.xsl
        CATALOG ${CMAKE_BINARY_DIR}/catalog.xml
        DIRECTORY man.manifest
        COMMENT "Generating man pages for Ryppl.${RYPPL_PROJECT_NAME}..."
        MAKE_TARGET ${RYPPL_PROJECT_NAME}-man)
...
endmacro(ryppl_add_documentation)
...
# Find the RypplBook DTD (it should be in the distribution!)
find_path(RYPPLBOOK_DTD_DIR rypplbook.dtd
  PATHS ${CMAKE_SOURCE_DIR}/tools/rypplbook/dtd
  DOC "Path to the RypplBook DTD")
mark_as_advanced(RYPPLBOOK_DTD_DIR)

# Find the RypplBook XSL stylesheets (they should be in the distribution!)
find_path(RYPPLBOOK_XSL_DIR docbook.xsl
  PATHS ${CMAKE_SOURCE_DIR}/tools/rypplbook/xsl
  DOC "Path to the RypplBook XSL stylesheets")
mark_as_advanced(RYPPLBOOK_XSL_DIR)
...
if (XSLTPROC_EXECUTABLE AND DOXYGEN)
  if (DOCBOOK_DTD_DIR AND DOCBOOK_XSL_DIR)
    ...
    configure_file(${CMAKE_SOURCE_DIR}/${RYPPL_CMAKE_DIR}/catalog.xml.in
