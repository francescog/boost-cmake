...
#                                                                        #
#   ryppl_library_project: Defines a Ryppl library project (e.g.,        #
#   Ryppl.Python).                                                       #
#                                                                        #
#   ryppl_add_library: Builds library binaries for Ryppl libraries       #
#   with compiled sources (e.g., ryppl_filesystem).                      #
#                                                                        #
#   ryppl_add_executable: Builds executables.                            #
##########################################################################

# Defines a Ryppl library project (e.g., for Ryppl.Python). Use as:
#
#   ryppl_library_project(libname
...
# library sources (for Ryppl libraries that build actual library
# binaries), and testdir1, testdir2, etc, are subdirectories
# containing regression tests. DEPENDS lists the names of the other
# Ryppl libraries that this library depends on. If the dependencies
...
#     TESTDIRS test
#     )
macro(ryppl_library_project LIBNAME)

  parse_arguments(THIS_PROJECT
    "SRCDIRS;TESTDIRS;EXAMPLEDIRS;HEADERS;DOCDIRS;DESCRIPTION;AUTHORS;MAINTAINERS;VERSION"
    ""
    ${ARGN}
    )

  trace(THIS_PROJECT_SRCDIRS)
  ...
  string(TOLOWER "${LIBNAME}" RYPPL_PROJECT_NAME)
  ...
  if (COMMAND cpack_add_component_group)
    ...
    if (THIS_PROJECT_DESCRIPTION)
      set(THIS_PROJECT_DESCRIPTION "Ryppl.${LIBNAME}\n\n${THIS_PROJECT_DESCRIPTION}")
  ...
  if(NOT EXISTS ${CMAKE_BINARY_DIR}/bin/tests/${RYPPL_PROJECT_NAME})
    file(MAKE_DIRECTORY ${CMAKE_BINARY_DIR}/bin/tests/${RYPPL_PROJECT_NAME})
  endif(NOT EXISTS ${CMAKE_BINARY_DIR}/bin/tests/${RYPPL_PROJECT_NAME})
  ...
  if ((BUILD_TESTS_INDEX GREATER -1) OR (BUILD_TESTS STREQUAL "ALL"))
    ...
    foreach(SUBDIR ${THIS_PROJECT_TESTDIRS})
      ...
      # add_subdirectory(${SUBDIR})
      set(RYPPL_${ULIBNAME}_TESTDIRS
	${RYPPL_${ULIBNAME}_TESTDIRS}
...
endmacro(ryppl_library_project)

macro(ryppl_tool_project TOOLNAME)
  parse_arguments(THIS_PROJECT
...
endmacro(ryppl_tool_project)

#TODO: Finish this documentation
# Defines dependencies of a ryppl project and testing targets. Use as:
#
#   ryppl_module(libname
...
macro(ryppl_module LIBNAME)
  ...
  # Export RYPPL_${LIBNAME}_DEPENDS
  string(TOUPPER "RYPPL_${LIBNAME}_DEPENDS" THIS_MODULE_LIBNAME_DEPENDS)
...
#   library target, e.g., ryppl_signals-mt-static.
#
...
#   multi-threaded, release variant of the library in Ryppl 1.34.0 as
...
#
macro(ryppl_library_variant_target_name)
  set(VARIANT_TARGET_NAME "")

  # The versioned name starts with the full Ryppl toolset
  if(WINMANGLE_LIBNAMES)
    set(VARIANT_VERSIONED_NAME "-${RYPPL_TOOLSET}")
  ...
  if(WINMANGLE_LIBNAMES)
    # Append the Ryppl version number to the versioned name
    if(RYPPL_VERSION_SUBMINOR GREATER 0)
      set(VARIANT_VERSIONED_NAME
	"${VARIANT_VERSIONED_NAME}-${RYPPL_VERSION_MAJOR}_${RYPPL_VERSION_MINOR}_${RYPPL_VERSION_SUBMINOR}")
    else(RYPPL_VERSION_SUBMINOR GREATER 0)
      set(VARIANT_VERSIONED_NAME 
	"${VARIANT_VERSIONED_NAME}-${RYPPL_VERSION_MAJOR}_${RYPPL_VERSION_MINOR}")
    endif(RYPPL_VERSION_SUBMINOR GREATER 0)
  endif(WINMANGLE_LIBNAMES)
endmacro(ryppl_library_variant_target_name)
...
#
#   ryppl_feature_interactions(prefix
#                              feature1 feature2 ...)
#
...
macro(ryppl_feature_interactions PREFIX)
...
endmacro(ryppl_feature_interactions)

# This macro is an internal utility macro that builds a particular
# variant of a ryppl library.
#
#   ryppl_library_variant(libname 
#                         feature1 feature2 ...)
#
# where libname is the name of the Ryppl library (e.g.,
# "ryppl_filesystem") and feature1, feature2, ... are the features
# that will be used in this variant. 
#
# This macro will define a new library target based on libname and the
# specific variant name (see ryppl_library_variant_target_name), which
...
macro(ryppl_library_variant LIBNAME)
  ...
  # message("ryppl_library_variant(${LIBNAME} ${ARGN})")

  # Handle feature interactions
  ryppl_feature_interactions("THIS_VARIANT" ${ARGN})
  ryppl_library_variant_target_name(${ARGN})
  ...
  if (THIS_VARIANT_OKAY)
    ...
    if (THIS_LIB_IS_STATIC)
      ...
      # different names, so we follow the Ryppl.Build version 2 style
	...
	COMPONENT Ryppl) #${LIB_COMPONENT})

      # set_property( 
      #      TARGET ${VARIANT_LIBNAME}
      #      PROPERTY RYPPL_CPACK_COMPONENT
      #      ${LIB_COMPONENT})
...
#     and will be extended each time ither ryppl_add_default_variant or 
#     ryppl_add_extra_variant is invoked. This macro will contain a list
...
#     that can be used for the ryppl_add_library macro, separated by
#     semicolons. For example, given the use of
#     ryppl_add_default_variant above, this variable will contain (at
#     least)
#
#        SINGLE_THREADED_COMPILE_FLAGS;SINGLE_THREADED_LINK_FLAGS;
#        MULTI_THREADED_COMPILE_FLAGS;MULTI_THREADED_LINK_FLAGS
#
#     When this variable is used in ryppl_add_library, it turns these
...
#
#   RYPPL_ADDLIB_OPTION_NAMES:
#     Like RYPPL_ADD_ARG_NAMES, this variable describes
#     feature-specific options to ryppl_add_library that can be used to
...
#
#   RYPPL_ADDEXE_OPTION_NAMES:
#     Like RYPPL_ADDLIB_OPTION_NAMES, except that that variable 
#     describes options to ryppl_add_executable that can be used to
...
macro(ryppl_add_default_variant)
  # Update RYPPL_DEFAULT_VARIANTS
  if (RYPPL_DEFAULT_VARIANTS)
    set(RYPPL_DEFAULT_VARIANTS_ORIG ${RYPPL_DEFAULT_VARIANTS})
    set(RYPPL_DEFAULT_VARIANTS)
    foreach(VARIANT ${RYPPL_DEFAULT_VARIANTS_ORIG})
      foreach(FEATURE ${ARGN})
        list(APPEND RYPPL_DEFAULT_VARIANTS "${VARIANT}:${FEATURE}")
      endforeach(FEATURE ${ARGN})
    endforeach(VARIANT ${RYPPL_DEFAULT_VARIANTS_ORIG})
    set(RYPPL_DEFAULT_VARIANTS_ORIG)
  else (RYPPL_DEFAULT_VARIANTS)
    set(RYPPL_DEFAULT_VARIANTS ${ARGN})
  endif (RYPPL_DEFAULT_VARIANTS)

  # Set Feature flag options used by the ryppl_library macro and the
  # RYPPL_FEATURES variable
  set(RYPPL_DEFVAR_FEATURES)
  foreach(FEATURE ${ARGN})
    set(RYPPL_ADD_ARG_NAMES 
      "${RYPPL_ADD_ARG_NAMES};${FEATURE}_COMPILE_FLAGS;${FEATURE}_LINK_FLAGS;${FEATURE}_LINK_LIBS")
    set(RYPPL_ADDLIB_OPTION_NAMES "${RYPPL_ADDLIB_OPTION_NAMES};NO_${FEATURE}")
    set(RYPPL_ADDEXE_OPTION_NAMES "${RYPPL_ADDEXE_OPTION_NAMES};${FEATURE}")
    if (RYPPL_DEFVAR_FEATURES)
      set(RYPPL_DEFVAR_FEATURES "${RYPPL_DEFVAR_FEATURES}:${FEATURE}")
    else (RYPPL_DEFVAR_FEATURES)
      set(RYPPL_DEFVAR_FEATURES "${FEATURE}")
    endif (RYPPL_DEFVAR_FEATURES)
  endforeach(FEATURE ${ARGN})
  list(APPEND RYPPL_FEATURES ${RYPPL_DEFVAR_FEATURES})
endmacro(ryppl_add_default_variant)

# Updates the set of "extra" build variants, which may be used to
# generate extra, library-specific variants of libraries.
#
#   ryppl_add_extra_variant(feature-val1 feature-val2 ...)
#
...
# variant, used by the Ryppl.Python library, which generates separate
# variants of the Ryppl.Python library: one variant uses the Python
# debug libraries, the other does not.
#
# The difference between ryppl_add_default_variant and
# ryppl_add_extra_variant is that adding a new default variant
# introduces additional variants to *all* Ryppl libraries, unless
...
#   RYPPL_ADDEXE_OPTION_NAMES:
#     See ryppl_add_default_variant.
macro(ryppl_add_extra_variant)
  set(RYPPL_EXTVAR_FEATURES)
  foreach(FEATURE ${ARGN})
    set(RYPPL_ADD_ARG_NAMES 
      "${RYPPL_ADD_ARG_NAMES};${FEATURE}_COMPILE_FLAGS;${FEATURE}_LINK_FLAGS;${FEATURE}_LINK_LIBS")
    set(RYPPL_ADDLIB_OPTION_NAMES "${RYPPL_ADDLIB_OPTION_NAMES};NO_${FEATURE}")
    set(RYPPL_ADDEXE_OPTION_NAMES "${RYPPL_ADDEXE_OPTION_NAMES};${FEATURE}")
    if (RYPPL_EXTVAR_FEATURES)
      set(RYPPL_EXTVAR_FEATURES "${RYPPL_EXTVAR_FEATURES}:${FEATURE}")
    else (RYPPL_EXTVAR_FEATURES)
      set(RYPPL_EXTVAR_FEATURES "${FEATURE}")
    endif (RYPPL_EXTVAR_FEATURES)
  endforeach(FEATURE ${ARGN})  
  list(APPEND RYPPL_FEATURES ${RYPPL_EXTVAR_FEATURES})
endmacro(ryppl_add_extra_variant)
...
# ryppl_add_executable or ryppl_add_library and what options the user
# has set.
macro(ryppl_select_variant NAME PREFIX)
  set(${PREFIX}_DEBUG_AND_RELEASE FALSE)
  set(SELECT_VARIANT_OKAY TRUE)
  set(SELECT_VARIANT_FAILURE_REASONS)
  set(${PREFIX}_VARIANT)

  foreach(FEATURESET_STR ${RYPPL_FEATURES})
  ...
  endforeach(FEATURESET_STR ${RYPPL_FEATURES})
  ...
  if (SELECT_VARIANT_OKAY)
    ...
    # Handle feature interactions
    ryppl_feature_interactions("${PREFIX}" ${${PREFIX}_VARIANT})
  else ()
    set(${PREFIX}_VARIANT)
  endif ()
endmacro(ryppl_select_variant)

# Creates a new Ryppl library target that generates a compiled library
...
#
# where libname is the name of Ryppl library binary (e.g.,
# "ryppl_regex") and source1, source2, etc. are the source files used
...
#   might want to add "-DRYPPL_SIGNALS_NO_LIB=1" through this option
...
#   for libraries external to Ryppl; for Ryppl libraries, use DEPENDS.
#
...
#
#   DEPENDS: States that this Ryppl library depends on and links
#   against another Ryppl library. The arguments to DEPENDS should be
#   the unversioned name of the Ryppl library, such as
#   "ryppl_filesystem". Like LINK_LIBS, this option states that all
...
#   Ryppl library against the same variant of the other Ryppl
#   library. For example, if the ryppl_mpi_python library DEPENDS on
#   ryppl_python, multi-threaded variants of ryppl_mpi_python will
#   link against multi-threaded variants of ryppl_python.
#
...
#     tss.cpp xtime.cpp
#     SHARED_COMPILE_FLAGS "-DRYPPL_THREAD_BUILD_DLL=1"
#     STATIC_COMPILE_FLAGS "-DRYPPL_THREAD_BUILD_LIB=1"
#     NO_SINGLE_THREADED
#   )
macro(ryppl_add_library LIBNAME)
  parse_arguments(THIS_LIB
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;EXTRA_VARIANTS;FORCE_VARIANTS;${RYPPL_ADD_ARG_NAMES}"
    "MODULE;NO_INSTALL;${RYPPL_ADDLIB_OPTION_NAMES}"
    ${ARGN}
    )
  ...
  #
  # cmake RypplConfig.cmake generation needs to know which
  # libraries are available
  #
  set(RYPPL_ALL_COMPONENTS ${LIBNAME} ${RYPPL_ALL_COMPONENTS} 
    PARENT_SCOPE)
  ...
  if (THIS_LIB_EXTRA_VARIANTS)
    ...
    foreach(VARIANT ${RYPPL_DEFAULT_VARIANTS})
    ...
    endforeach(VARIANT ${RYPPL_DEFAULT_VARIANTS})
  else (THIS_LIB_EXTRA_VARIANTS)
    set(THIS_LIB_VARIANTS ${RYPPL_DEFAULT_VARIANTS})
  ...
  foreach(VARIANT_STR ${THIS_LIB_VARIANTS})
    ...
    ryppl_library_variant(${LIBNAME} ${VARIANT})
...
endmacro(ryppl_add_library)

# Creates a new executable from source files.
#
#   ryppl_add_executable(exename
#                        source1 source2 ...
#                        [COMPILE_FLAGS compileflags]
#                        [feature_COMPILE_FLAGS compileflags]
#                        [LINK_FLAGS linkflags]
...
#   ryppl_add_executable (see the "feature" argument description,
...
#   external to Ryppl; for Ryppl libraries, use DEPENDS.
#
...
#   a Ryppllibrary. The arguments to DEPENDS should be the unversioned
#   name of the Ryppl library, such as "ryppl_filesystem". Like
...
#   the appropriate variant of a Ryppl library. For example, if the
#   MULTI_THREADED feature was requested in the call to
#   ryppl_add_executable, DEPENDS will ensure that we only link
...
#
#   NO_INSTALL: Don't install this executable with the rest of Ryppl.
#
...
#     DEPENDS ryppl_wave ryppl_program_options ryppl_filesystem 
#             ryppl_serialization
#     )
macro(ryppl_add_executable EXENAME)
  # Note: ARGS is here to support the use of ryppl_add_executable in
  # the testing code.
  parse_arguments(THIS_EXE
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;OUTPUT_NAME;ARGS;${RYPPL_ADD_ARG_NAMES}"
    "NO_INSTALL;${RYPPL_ADDEXE_OPTION_NAMES}"
    ${ARGN}
    )
  ...
  # ryppl_add_executable and what options the user has set.
  ryppl_select_variant(${EXENAME} THIS_EXE)
  ...
  if (THIS_PROJECT_IS_TOOL)
    ...
    set(THIS_EXE_NAME ${RYPPL_PROJECT_NAME}-${EXENAME})
  endif()
  ...
  if (THIS_EXE_DEBUG_AND_RELEASE)
    ryppl_library_variant_target_name(RELEASE ${THIS_EXE_VARIANT})
    set(RELEASE_VARIANT_TARGET_NAME "${VARIANT_TARGET_NAME}")
    ryppl_library_variant_target_name(DEBUG ${THIS_EXE_VARIANT})
    set(DEBUG_VARIANT_TARGET_NAME "${VARIANT_TARGET_NAME}")
  else (THIS_EXE_DEBUG_AND_RELEASE)
    ryppl_library_variant_target_name(${THIS_EXE_VARIANT})
  ...
  foreach(LIB ${THIS_EXE_DEPENDS})
    if (LIB MATCHES ".*-.*")
      ...
      # e.g., "ryppl_filesystem". We add on the appropriate variant
      # name(s).
...
endmacro(ryppl_add_executable)


# Like ryppl_add_library, but builds a single library variant
# FIXME: I'm not sure if I like this or not. Document it if it survives.
macro(ryppl_add_single_library LIBNAME)
  parse_arguments(THIS_LIB
    "DEPENDS;COMPILE_FLAGS;LINK_FLAGS;LINK_LIBS;${RYPPL_ADD_ARG_NAMES}"
    "NO_INSTALL;MODULE;${RYPPL_ADDEXE_OPTION_NAMES}"
    ${ARGN}
    )
  set(THIS_LIB_SOURCES ${THIS_LIB_DEFAULT_ARGS})

  ryppl_select_variant(${LIBNAME} THIS_LIB)
  trace(THIS_LIB_VARIANT)
  if (THIS_LIB_VARIANT)
    add_custom_target(${LIBNAME})
    separate_arguments(THIS_LIB_VARIANT)
    ryppl_library_variant(${LIBNAME} ${THIS_LIB_VARIANT})
  endif ()
endmacro(ryppl_add_single_library)


#
#  Macro for building ryppl.python extensions
#
macro(ryppl_python_extension MODULE_NAME)
